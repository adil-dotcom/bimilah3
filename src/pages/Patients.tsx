...
export default function Patients() {
  ...
  // Synchroniser les données des patients avec les rendez-vous
  const updatedPatients = useMemo(() => {
    return patients.map(patient => {
      ...
      if (patientAppointments.length === 0) return patient;

      // Trier les rendez-vous par date
      const sortedAppointments = patientAppointments.sort((a, b) => 
        parseISO(b.time).getTime() - parseISO(a.time).getTime()
      );

      // Dernier rendez-vous
      const lastAppointment = sortedAppointments[0];
      
      // Prochain rendez-vous (premier rendez-vous futur)
      const nextAppointment = sortedAppointments.find(apt => 
        parseISO(apt.time) > new Date()
      );

      return {
        ...patient,
        derniereConsultation: lastAppointment ? format(parseISO(lastAppointment.time), 'dd/MM/yyyy') : patient.derniereConsultation,
        prochainRdv: nextAppointment ? format(parseISO(nextAppointment.time), 'dd/MM/yyyy HH:mm') : '-',
        nombreConsultations: patientAppointments.filter(apt => !apt.isCanceled).length
      };
    });
  ...
  const filteredPatients = useMemo(() => {
    return updatedPatients.filter(patient => {
      const matchesSearch = 
  ...
  }, [updatedPatients, searchTerm, dateRange]);
