...
export default function Billing() {
  ...
  // Filtrer les rendez-vous avec un montant défini (incluant 0)
  const paidAppointments = useMemo(() => {
    return appointments
      .filter(apt => {
        // Vérifier si le montant est défini (même 0)
        return apt.amount !== undefined && apt.amount !== null;
      })
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredAppointments.map((appointment) => {
                const amount = parseFloat((appointment.amount || '0').replace(',', '.'));
                const isGratuit = amount === 0;
                  ...
                  <tr key={appointment.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                        ...
                        {isGratuit ? 'Gratuit' : appointment.paid ? 'Payé' : 'Non payé'}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {isGratuit ? '-' : appointment.paymentMethod}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
